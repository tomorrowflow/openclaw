      ...
      try {
        switch (key) {
          ...
          case "lang":
            ...
            overrides.kokoro = { ...overrides.kokoro, lang: rawValue };
            break;
          case "kokoro_voice":
          case "kokorovoice":
            if (!policy.allowVoice) break;
          ...
          case "kokorospeed":
            if (!policy.allowVoiceSettings) break;
            {
              const value = parseNumberValue(rawValue);
              if (value == null) {
                warnings.push("invalid kokoro speed value");
                break;
              }
              requireInRange(value, 0.25, 4, "kokoro speed");
              overrides.kokoro = { ...overrides.kokoro, speed: value };
            }
            break;
          default:
            break;
