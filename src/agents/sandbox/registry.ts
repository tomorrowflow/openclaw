...
function containerEntryToRow(entry: SandboxRegistryEntry, existing?: SandboxRegistryEntry | null) {
  const next: SandboxRegistryEntry = {
    ...
    // Creation supplies these; a lastUsedAt touch omits them and must not erase
    // them, while a recreate under the same name legitimately replaces them.
    mounts: entry.mounts ?? existing?.mounts,
  };
  return {
