...
export const ELEMENT_TOOLS: ToolSpec[] = [
  ...
  },
  {
    ...
    handler: (p, api) => {
      const qs = typeof p.type === "string" && p.type ? `?type=${encodeURIComponent(p.type)}` : "";
  ...
  },
  {
    ...
    handler: async (p, api) => {
      ...
      const ids = Array.isArray(p.elementIds) ? p.elementIds.map((value) => String(value)) : [];
      const offsetX = typeof p.offsetX === "number" ? p.offsetX : 20;
      const offsetY = typeof p.offsetY === "number" ? p.offsetY : 20;
